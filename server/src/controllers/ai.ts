...
export const generateInterview = TryCatch(async(req:AuthenticatedRequest, res)=>{
    ...
    if(!mode || !round) return res.status(400).json({ message: "Mode and round are Reuired"});
    if(mode === "manual" && (!skills?.length || !experience?.trim()))
        return res.status(400).json({
         message: "Skills and experience are required"
    });
...
export const buildResume = TryCatch(async (req: AuthenticatedRequest, res)=>{
    ...
    const parts: any[] = [{text: buildResumePrompt(mode, formData)}]// any:mtlb arry me kisi bhi type ka data aa skta h eh text,image,string, object,array,function, "TypeScript, type checking मत करो"
